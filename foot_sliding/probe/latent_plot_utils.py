...
def plot2D(data, labels, title):
    ...
    fig.tight_layout()
    tikzplotlib.save("%s.tex" % title, figure=fig, strict=True, encoding='utf8')
...
def calc_pca_curve(raw):
    ...
    name = "pca_curve"
    tikzplotlib.save(name + ".tex", figure=fig, strict=True, encoding='utf8')
...
def calc_many_blas(raws, calc_single):
    ...
    whole = np.concatenate(raws, axis=0)  # style_code: (3016, 144) / style_adain: (3016, 288)
